...
const PayoutHistory = () => {
  ...
  const uplineDisplayCols = ['ADMIN', 'TDS', 'UPLINE TOTAL', ...uplineColNames];
  const formatHeader = (key) => key.replace(/([A-Z])/g, ' $1').toUpperCase();
  const displayColumns = dynamicColumns.length > 0
    ? ['SNO', ...dynamicColumns.map(formatHeader), ...uplineDisplayCols]
    : ['SNO', 'DATE & TIME', 'MEMBER DETAIL', 'BANK NAME', 'ACCOUNT NO', 'AMOUNT', 'CHARGES', 'TOTAL', 'STATUS', 'RECEIPT', ...uplineDisplayCols];
