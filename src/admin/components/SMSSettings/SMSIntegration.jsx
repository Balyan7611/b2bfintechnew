...
import { 
  FiSearch, FiEdit, FiTrash2, FiPlus, FiChevronLeft, FiChevronRight, FiDatabase, FiX, FiCheck, FiCpu, FiGlobe, FiKey, FiLink
} from 'react-icons/fi';
    ...
    <div className={styles.container}>
      ...
      <div className={styles.cardFullMobile}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 25px', borderBottom: '1px solid #F1F5F9', flexWrap: 'nowrap', gap: '15px' }}>
          <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, color: '#0D1B3E', whiteSpace: 'nowrap' }}>Gateway Integration</h3>
          ...
          <table className={styles.table} style={{ minWidth: '1300px' }}>
            <thead>
              <tr style={{ background: 'linear-gradient(90deg, #0D1B5E 0%, #1a2f8a 100%)' }}>
                ...
                <th style={{ width: '250px' }}>PROVIDER IDENTITY</th>
                <th style={{ width: '150px', textAlign: 'center' }}>API TYPE</th>
                <th style={{ width: '400px' }}>GATEWAY ENDPOINT</th>
                ...
                <tr key={gtw.id} className={styles.hoverRow}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                      <button className={styles.editBtn} onClick={() => setIsModalOpen(true)} title="Edit Configuration" style={{ width: '32px', height: '32px' }}><FiEdit /></button>
                      <button className={styles.deleteBtn} style={{ background: 'rgba(229, 62, 62, 0.08)', color: '#E53E3E', border: 'none', width: '32px', height: '32px', borderRadius: '8px' }} title="Remove Gateway"><FiTrash2 /></button>
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#718096', fontSize: '0.8rem', fontWeight: 600 }}>
                       <FiLink style={{ color: '#A0AEC0' }} />
                       {gtw.endpoint}
                    </div>
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <span style={{ background: gtw.status ? 'rgba(39, 174, 96, 0.1)' : 'rgba(229, 62, 62, 0.1)', color: gtw.status ? '#27AE60' : '#E53E3E', padding: '6px 16px', borderRadius: '50px', fontSize: '0.7rem', fontWeight: 800, border: gtw.status ? '1px solid rgba(39, 174, 96, 0.2)' : '1px solid rgba(229, 62, 62, 0.2)' }}>
                      ● {gtw.status ? 'Active' : 'Offline'}
                    </span>
                  </td>
                  <td style={{ textAlign: 'right', fontWeight: 700, color: '#718096', fontSize: '0.8rem', paddingRight: '25px' }}>{gtw.addDate}</td>
                </tr>
              ))}
            </tbody>
          </table>
